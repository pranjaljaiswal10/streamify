...
const WatchvideoDescription = ({description,publishedAt,viewCount}) => {
  ...
  return (<div className="bg-slate-100 hover:bg-slate-300 p-4 mb-8 rounded text-sm md:text-base">
  <span className="font-bold">{formatter.format(viewCount)}</span>
  <span className="px-4 font-bold">{relativeTime(publishedAt)}</span>
  <br/>
 {isOpen? <p className="text-sm whitespace-pre-line">{description}<span onClick={handlClick} className="font-bold cursor-pointer pl-2">see less </span></p>
